...
fastify.post('/', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
      ...
      const rs: any = await geo_model.login(db, username, encPassword)
      if (rs.length > 0) {
        const user: any = rs[0]
          console.log(user)
...
fastify.post('/singup', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    const rs_email: any = await geo_model.validation_email(db, email)
    if (rs_email.length > 0) {
        ...
        return // reply.sent = true  //exit(); to stop the function execution   ออกจากลูปการทำงาน 
    }const rs_username: any = await geo_model.validation_username(db, username)
    ...
    try {
      ...
      await geo_model.create(db, data)
     // reply.send({ message: 'Insert data', status: true })
      ...
      console.log(error)
        ...
        const lastrs: any = await geo_model.lastidread(db)
       ...
       // reply.code(200).send({ array: data_array })
        ...
        await geo_model.updateuid(db, user_idx, data_array)
        const rs: any = await geo_model.login(db, username, encPassword)
        if (rs.length > 0) {
        const user: any = rs[0]
          console.log(user)
...
fastify.post('/resetpass', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
      ...
      const rs: any = await geo_model.resetPassword(db, datareset)
      if (rs.length > 0) {
        const user: any = rs[0] 
          reply.send({
              status: true, code: 200,
...
fastify.post('/changepassword', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
      ...
      const rsold: any = await geo_model.login(db, username, encoldpassword)
        if (rsold.length > 0) {

            const data_array: any = {}; 
            data_array.password = encnewpassword
            await geo_model.where_user_update_password(db, username, data_array)
      ...
      const rs: any = await geo_model.login(db, username, encnewpassword)
      if (rs.length > 0) {
        const user: any = rs[0]
          console.log(user)
           /******************************ตรวจสอบวันหมดอายุ Token check*************************************/
...
fastify.post('/activecode', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
        ...
        const profile: any = await geo_model.profile(db, user_id)
        ...
        if (timestamp_culs > time_settings) {
            ...
            const data_array: any = {};
            data_array.status = 1
            await geo_model.updateuid(db, user_id, data_array)
