...
import querystringysingin from '../../../modules/administrator/schemas/querystringysingin'
/************* validate schemas*******************/
...
export default async function auth(fastify: FastifyInstance) {
    ...
    fastify.post('/singup', {
        ...
        var passwordvd = body.password
        var email = body.email
        var role_id : any= body.role_id
        var network_id = body.network_id
        ...
        const status_insert = 0
            ...
            /*******************/
            const validation_user_roles: any = await AuthModels.validation_administrator_roles(db1, user_idx)
            if (validation_user_roles.length > 0) {    
            } else {
                const dataroes: any = {}
                const date = new Date()
                dataroes.user_id = user_idx
                dataroes.role_id = 3
                dataroes.create = date
                dataroes.update = date
                var create_roles: any = await AuthModels.create_roles(db1, dataroes)
                console.log('create_roles ', create_roles);
                const data_history: any = {} 
                data_history.user_id = user_idx
                data_history.type_id = 1
                data_history.date = date
                data_history.remark = ' Register account system , ลงทะเบียน ใช้งานระบบ '
                data_history.status = 1
                var create_history: any = await AuthModels.create_history(db1, data_history)
                        ...
                        try {
                            ...
                            // const html: any = ejs.render(fs.readFileSync(templateFile, 'utf8'), {  title: 'Data Reset Password',url: 'https://www.my.com?token='+token, token: token,})
                                ...
                                const sendMail = await transporter.sendMail({
                                    ...
                                    to: user.email,
                                    subject: 'Activate account ' + full_name,
                                    text: 'Activate account ' + full_name,
                                    html: html
                                })
                                reply.code(200).send({
                                    title: { status: true, statusCode : 200,},
                                    message: 'Register successfully ! Please check email and  activate your account',
                                    message_th: 'ลงทะเบียนสำเร็จ ! โปรดตรวจสอบอีเมลและเปิดใช้งานบัญชีของคุณ',
                                    enc_user_idx: enc_user_idx,
                                    token
                                })
                            return
            ...
            return //reply.sent = true // exit loop ออกจากลูปการทำงาน 
                        ...
                        try {
                                ...
                                // const html: any = ejs.render(fs.readFileSync(templateFile, 'utf8'), {  title: 'Data Reset Password',url: 'https://www.my.com?    token='+token, token: token,})
                                    ...
                                    from: '"App service" <admin@appservice.com>',
                                    to: email,
                                    subject: 'Activate account ' + fullname,
                                    text: 'Activate account ' + fullname,
                                    html: html
                                })
                                reply.code(200).send({
                                    title: { status: true, statusCode : 200,},
    ...
    /**************************************************/ 
    fastify.get('/singindev', {
        schema: querystringysingin // validate schemas
        }, async (request: FastifyRequest, reply: FastifyReply) => {
        ...
        var state: any = getRandomString(32)
        try {
            if (username == '') {
                reply.header('version', 1)
                reply.header('x-cache-status', 0) // 1=yes ,0=no
        ...
        if (rs.length > 0) {
            ...
            /******************************Crate Token *************************************/
                ...
                */
                ...
                const set_cookie2: any = 'state=' + state + '; Max-Age=' + MaxAge + '; SameSite=None; Secure';
    ...
    fastify.post('/resetpass', {
        ...
        try {
            if (datareset === "") {
                ...
                reply.header('code', 500)
                reply.header('status', false)
                reply.header('token_expire_setting', token_expire_setting_in)
                reply.header('token_expire_setting_msg', token_expire_setting_msg)
                ...
                if (token_expire_setting == 1) {
                        var token = fastify.jwt.sign({
                                    ...
                                    at: { 
                                        issued_at: issued_at,
                                        timeconfig: TIMEEXPIRE,
                                        TIMEEXPIRE_TOKEN: TIMEEXPIRE_TOKEN_resetPassword1,
                                        state: getRandomString(32),  
                                    },                  
