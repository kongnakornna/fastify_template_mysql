...
fastify.post('/', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
      ...
      const rs: any = await AdminModel.login(db, username, encPassword)
      if (rs.length > 0) {
        const user: any = rs[0]
          console.log(user)
...
fastify.post('/singup', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    const rs_email: any = await AdminModel.validation_email(db, email)
    if (rs_email.length > 0) {
        ...
        reply.sent = true // I tried that, didn't work  ออกจากลูปการทำงาน 
    }const rs_username: any = await AdminModel.validation_username(db, username)
    ...
    try {
      ...
      await AdminModel.create(db, data)
     // reply.send({ message: 'Insert data', status: true })
      ...
      console.log(error)
        ...
        const lastrs: any = await AdminModel.lastidread(db)
       ...
       // reply.code(200).send({ array: data_array })
        ...
        await AdminModel.updateuid(db, user_idx, data_array)
        const rs: any = await AdminModel.login(db, username, encPassword)
        if (rs.length > 0) {
        const user: any = rs[0]
          console.log(user)
...
fastify.post('/resetpass', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
      ...
      const rs: any = await AdminModel.resetPassword(db, datareset)
      if (rs.length > 0) {
        const user: any = rs[0] 
          reply.send({
              status: true, code: 200,
...
fastify.post('/changepassword', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
      ...
      const rsold: any = await AdminModel.login(db, username, encoldpassword)
        if (rsold.length > 0) {

            const data_array: any = {}; 
            data_array.password = encnewpassword
            await AdminModel.where_user_update_password(db, username, data_array)
      ...
      const rs: any = await AdminModel.login(db, username, encnewpassword)
      if (rs.length > 0) {
        const user: any = rs[0]
          console.log(user)
           /******************************ตรวจสอบวันหมดอายุ Token check*************************************/
...
fastify.post('/activecode', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
        ...
        const profile: any = await AdminModel.profile(db, user_id)
        ...
        if (timestamp_culs > time_settings) {
            ...
            const data_array: any = {};
            data_array.status = 1
            await AdminModel.updateuid(db, user_id, data_array)
