        ...
        const user: any = rs[0]
          console.log(user)
           /******************************ตรวจสอบวันหมดอายุ Token check*************************************/
            const day = 1;
            const TIMEEXPIRE =process.env.TIMEEXPIRE;
            const time_expire_set = TIMEEXPIRE;
            const time_expire_set1 = 60 * 5;
            const time_setting = 86000 || process.env.TIMEEXPIRE;
     ...
     // reply.send({ message: 'Insert data', status: true })
      ...
      console.log(error)
        ...
        const lastrs: any = await userModel.lastidread(db1)
           // reply.code(500).send({ da: lastrs }) return //reply.sent = true // I tried that, didn't work  ออกจากลูปการทำงาน 
        ...
        const user: any = rs[0]
          console.log(user)
           /******************************ตรวจสอบวันหมดอายุ Token check*************************************/
            ...
            var time_setting = 86000 || process.env.TIMEEXPIRE;
            var today = new Date();
        ...
        const user: any = rs[0]
          console.log(user)
           /******************************ตรวจสอบวันหมดอายุ Token check*************************************/
            ...
            var time_expire_set1 = 60 * 5;
            var timesetting : any = process.env.TIMEEXPIRE;
            var today = new Date();
            var date = today.getFullYear()+'-'+(today.getMonth()+1)+'-'+today.getDate();
            var time = today.getHours() + ":" + today.getMinutes() + ":" + today.getSeconds();
            var dateTime = date + ' ' + time;
